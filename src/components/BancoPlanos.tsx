...
export default function BancoPlanos({ session }) {
            ...
            // Auto-redirect para posAula removido — AgendaView unifica desktop e mobile
            // O botão "Registrar" em cada AulaCard cobre o caso de uso de pós-aula
            ...
            useEffect(() => {
                ...
                // Usa syncDelta (sem DELETE global) para evitar timeout 57014 com muitos planos
                ...
                <div className="h-screen bg-[#F6F8FB] dark:bg-[#0F172A] flex overflow-hidden">
                        ...
                        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={fecharModal}>
                            <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-xl" onClick={(e)=>e.stopPropagation()}>
                                ...
                                <div className="p-6 space-y-5">
                                    ...
                                    {/* Habilidades BNCC — colapsável */}
                                    {planoSelecionado.habilidadesBNCC && planoSelecionado.habilidadesBNCC.length > 0 && (
                                        <div>
                                            <button onClick={()=>setBnccExpanded(v=>!v)}
                                                className="flex items-center gap-2 w-full text-left group">
                                                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">🏛️ Habilidades BNCC</p>
                                                <span className="text-xs bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded-full">{planoSelecionado.habilidadesBNCC.length}</span>
                                                <svg className={`w-3.5 h-3.5 text-slate-400 ml-auto transition-transform ${bnccExpanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7"/></svg>
                                            </button>
                                            {bnccExpanded && (
                                                <div className="space-y-1.5 mt-2">{planoSelecionado.habilidadesBNCC.map((hab, i) => { const [cod, ...resto] = hab.split(' - '); return (<div key={i} className="flex gap-2 items-start bg-slate-50 border border-slate-200 rounded-xl px-3 py-2"><span className="text-xs font-bold text-blue-600 shrink-0 mt-0.5">{cod}</span><span className="text-sm text-slate-700">{resto.join(' - ')}</span></div>); })}</div>
                                            )}
                                        </div>
                                    )}
                                            ...
                                            <div className="flex flex-col gap-2">
                                                {(planoSelecionado.notasAdaptacao || []).map(nota => (
                                                    <div key={nota.id} className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
                                                        <p className="text-xs font-semibold text-amber-800 mb-1">{nota.turmaNome}</p>
                                    ...
                                    <div className="flex flex-col gap-2 pt-4 border-t border-slate-100">
                                        <button onClick={()=>editarPlano(planoSelecionado)} className="w-full border border-slate-300 hover:border-slate-400 hover:bg-slate-50 text-slate-600 hover:text-slate-800 py-3 rounded-xl font-semibold text-sm transition">Editar plano</button>
                                        <button onClick={()=>excluirPlano(planoSelecionado.id)} className="w-full border border-red-200 hover:bg-red-50 text-red-400 hover:text-red-600 py-2.5 rounded-xl font-medium text-sm transition">Excluir</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <nav className="fixed bottom-0 left-0 right-0 sm:hidden bg-white dark:bg-[#1F2937] border-t border-slate-200 dark:border-[#374151] flex justify-around items-center z-40"
                        ...
                        <button
                            ...
                            className="flex flex-col items-center gap-0.5 px-1 py-2 transition active:scale-95 min-w-0 flex-1 text-slate-400 dark:text-[#6B7280]"
                            ...
                            <span className="text-[10px] font-medium leading-tight">{darkMode ? 'Claro' : 'Escuro'}</span>
                        </button>
                    </nav>

                    <ModalConfiguracoes />
                    {viewMode !== 'posAula' && <ModalRegistroPosAula onSaved={() => setViewMode('agenda')} />}
                    <ModalGestaoTurmas />
