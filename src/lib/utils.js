...
// Gera ID único e estável para cada item.
...
export async function syncToSupabase(tabela, itens, userId, onStatus) {
    ...
    for (let tentativa = 1; tentativa <= MAX_TENTATIVAS; tentativa++) {
        try {
            ...
            // Estratégia segura: upsert de todos os itens + deletar os que sumiram
            if (itens && itens.length > 0) {
                const rows = itens.map(item => {
                    // CORREÇÃO: nunca usa Math.random() como ID.
                    // Se o item não tem id, gera um UUID estável e o persiste no objeto.
                    // Isso garante que o mesmo item sempre tenha o mesmo item_id no Supabase.
                    if (!item.id) {
                ...
                // CORREÇÃO: usa array nativo do Supabase em vez de string SQL manual,
                // evitando risco de injeção e quebra com IDs que contenham caracteres especiais.
            ...
            console.warn(`[MusiLab] Tentativa ${tentativa}/${MAX_TENTATIVAS} falhou para '${tabela}':`, e.message || e);
...
export async function syncConfiguracoes(cfg, userId, onStatus) {
    try {
        ...
        console.warn('[MusiLab] Erro sync configuracoes:', e.message || e);
        if (onStatus) onStatus('erro');
        return false;
    }
}

export async function loadFromSupabase(tabela, userId) {
    try {
        ...
        console.error('[MusiLab] Erro ao carregar ' + tabela + ':', e.message || e);
        return null;
    }
}

export async function loadConfiguracoes(userId) {
    try {
        const { data, error } = await supabase
            .from('configuracoes')
            .select('data')
